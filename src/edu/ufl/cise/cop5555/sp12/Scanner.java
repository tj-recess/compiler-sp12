...
public class Scanner {
    ...
    /**
     * points to next char to process during scanning or none if past the end of array
     */
    private int index;
        ...
        do{
            ...
            case START:
                ...
                case '|':
                    ...
                    break;
                case '0':
                    t = stream.new Token(Kind.INTEGER_LITERAL, beginOffset, index);
                    break;
            ...
            case IDENT_PART:
                if(!Character.isJavaIdentifierPart(ch) || ch == 26) //EOF = 26
                {
                    ...
                    {
                        t = stream.new Token(Kind.BOOLEAN_LITERAL, beginOffset, index);
                    }
                    else
                    {
                        ...
                        if(keywordType != null)
                        {
                            t = stream.new Token(keywordType, beginOffset, index);
                        }
                        else
                        {
     ...
     * @param begin index in the array from where token should be read
     * @param end index in array until where token should be read
     * @return keywordType which will store the value of keyword in terms of Kind enumeration
     */
    private Kind getKeyword(int begin, int end)
    {
        Kind keywordType = this.keywordMap.get(getStringFromInputChars(begin, end, stream.inputChars));
        return keywordType;
    }

    /**
     * ad hoc method to convert some part of char[] array into string
     * @param begin starting index in array
     * @param end value next to last index of string in array
     * @param array from which chars are chosen
    ...
    /**
     * returns true if word represented by input character array is a boolean literal (true/false)
     * @param begin location from where word should be read
     * @param end location until where the word should be read
     * @return true if word is a boolean literal
     */
    private boolean isBooleanIdentifier(int begin, int end)
    {
        String actualWord = getStringFromInputChars(begin, end, stream.inputChars);
    ...
    /**
     * This method is called when we read an extra character and need to move index back by 1 pointer.
     */
