...
public class TestScanner
{
    ...
    // This expected values of the text of the input tokens are delineated by
    // '~'. Thus this method cannot
    // handle programs containing '~' in comments or strings. Fixing this is not
    // worth the trouble.
    ...
    {
        String input = "\u001a"; // control z at end of file should be ignored,
                                 // and yield just an EOF token
    ...
    {
        String input = "prog"; // prog is a keyword, should see a PROG token
                               // with text prog, followed by EOF
    ...
    {
        String input = "xyz"; // this is an identifier with text xyz followed by
                              // an EOF token
    ...
    {
        String input = "01"; // this is a 0 token followed by a 1 token followed
                             // by an EOF token. 01 is not legal in the
                             // language,
    ...
    public void testErrorInput0() throws IllegalStringLiteralException
    {
        ...
        Kind[] expectedKinds = { IDENTIFIER, IDENTIFIER, ILLEGAL_CHAR,
                INTEGER_LITERAL, EOF };
        TokenStream stream = getInitializedTokenStream(input);
        // compareText(stream, expected); We won't worry about exactly what text
        // goes into error tokens.
        compareKinds(stream, expectedKinds);
    }
    ...
    {
        String input = "\"abc\\\""; // This is tricky. Removing the escapes in
                                    // the String needed for Java, we have input
                                    // "abc\"
                                    // including the quotes. "abc\" is the what
                                    // we would have if the input were going to
                                    // be read
                                    // from a file. However, the final quote is
                                    // escaped so it is a quote within the
                                    // string
                                    //
